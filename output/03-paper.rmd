...
knitr::read_chunk('../01-setup.rmd')
knitr::read_chunk('../02-graphs.rmd')
```
...
In addition to `knitr` [@knitr] for generating this paper, `knitr::kable` and `kableExtra` [@kableExtra] were indispensable for tabular formatting. And, of course, we are thoroughly indebted to @rohan's book and associated course, Telling Stories with Data for instruction on how to put all the parts together into a cogent paper.
